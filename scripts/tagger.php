...
$re1 = array(
  ...
  '@(\p{Lu})1(\p{Lu})@u' => "$1I$2",
  '@(\p{Lu})—(\p{Lu})@u' => "$1-$2",
  '@\\\.</emph>@u' => '</emph>].', // etymologie
  '@S\. m\.@u' => "s. m.", // substantif masculin
  '@adj,@u' => "adj.", // adjectif
  '@ail\. @u' => "all. ",
  '@—[  ]*(\d+)@u' => "-$1", // tirets -15
  '@<hi rend="sup">[z]</hi>—@u' => "’-", // P<hi rend="sup">z</hi>—<emph>p</emph>
  '@<emph>([A-Za-z][’\'·]?)</emph>@u' => "$1", // tant pis pour l’italique sur une lettre dans les formules
  ...
  '@<emph>([^\pL]*)</emph>@u' => "$1", // nettoyer l’italique vide
  '@<p rend="[^"]*legendedelimage[^"]*">(.*?)</p>@u' => "<figDesc>$1</figDesc>", // Légende 
  '@<hi rend="sc">Fig.</hi>@u' => "Fig.",
...
function rewrite1()
{
  $fp = fopen($HOME ."xml/medict37020d.xml", "w");
  fwrite($fp, '<?xml version="1.0" encoding="UTF-8"?>
  <?xml-model href="../schema/littre1873.rng" type="application/xml" schematypens="http://relaxng.org/ns/structure/1.0"?>
  ...
  foreach (glob($HOME . "rawx/*.xml") as $srcfile) {
    echo $srcfile . "\n";
